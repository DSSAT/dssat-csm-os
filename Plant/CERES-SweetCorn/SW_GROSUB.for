...
!  03/06/2006 JIL Modified to simulate sweet corn
!  10/19/2007 CHP Added RS canopy height computation
...
!     4/21/2008 Read fresh weight option from species file
      CHARACTER*1 ISWFWT
...
!          XFWYLDFCY = 0.0
     ...
     &      PStres1, PStres2, PUptake, FracRts)             !Output
          ...
          CALL SW_FreshWt (ISWITCH, ISWFWT,
...
!     &              AMIN1(TURFAC,(1.0-SATFAC),PStres2,AGEFAC)
              PLAG   = 3.0*XN*XN*TI*
     &              AMIN1(TURFAC,(1.0-SATFAC),PStres2)
...
!             CHP 9/5/04 Added Pstres2
              PLAG   = 3.5*XN*XN*TI
     &                    *AMIN1(AGEFAC,TURFAC,(1.0-SATFAC),PStres2)
...
!                  PLAG   = 3.5*XN*XN*TI*AMIN1(AGEFAC,TURFAC,(1.-SATFAC))
                  PLAG   = 3.5*XN*XN*TI
     &                *AMIN1(AGEFAC,TURFAC,(1.-SATFAC),PStres2)
...
!      IF (ISTAGE .GE. 3 .AND. ISWITCH%IDETG == 'Y') THEN
!        OPEN(UNIT=NOUTDG,FILE=OUTG,STATUS='UNKNOWN',
!     &       ACCESS='APPEND')
!        WRITE (NOUTDG,1100) DOY,EARDMC,EARFWT,EARWT,FWYIELD,
!     &                      MKTFWYLD,EARMKT
!      ENDIF
...
!     Format Strings
!----------------------------------------------------------------------
 1000 FORMAT (' DOY EARDMC EARFWT EARDW  FWYIELD MKTFWYLD   EARMKT')
!     &        'FWYLDFCY FWYLD1+2   EARFCY   EARUS1+2')
 1001 FORMAT (6X,'(0-1)  g/ear g/ear  <---- kg/ha ---> < ear/ha >')
!     &        '------------->  <---- ear/ha ---->')
! 1100 FORMAT (I4,X,F6.1,X,F6.3,X,F6.1,X,F5.1,X,6(F8.1,X),F6.1)
! 1100 FORMAT (I4,X,F6.3,X,F6.1,X,F5.1,X,5(F8.1,X),F10.1)
 1100 FORMAT (I4,1X,F6.3,1X,F6.1,1X,F5.1,1X,3(F8.1,1X))
 2700 FORMAT (2X,'Crop mature on JD',I4,' due to slowed grain filling')
 2800 FORMAT (2X,'Crop failure growth program terminated ')


      END SUBROUTINE SW_GROSUB
