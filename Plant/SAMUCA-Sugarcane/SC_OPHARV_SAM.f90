...
!      PlantStres % StageName(2) = 'Emergence -stalk emerg.'
!      PlantStres % StageName(3) = 'Stalk emerg. -Harvest  '
...
!      MJ, Mar 2008: replaced the line below with
!      PlantStres % W_grow = 1.0  !TURFAC
!     with: (SWDF2)
      PlantStres % W_grow = PStres2
!      and (SWDF1):
!      PlantStres % W_phot = 1.0  !SWFAC 
...
!     and the plant is emerged, and this is after STGDOY(4)
      IF ((YRDOY >= YRPLT) .AND. (EMERGED) .AND. (YRDOY >= STGDOY(4)) .AND. (STGDOY(4) .NE. 0) &
	        .AND. (STGDOY(11) .NE. 0) .AND. (YRDOY .GE. STGDOY(11)))THEN
...
!     peak pop has NOT yet been reached.
      IF ((YRDOY >= YRPLT) .AND. (EMERGED) .AND. (YRDOY >= STGDOY(4)) .AND.	(STGDOY(4) .NE. 0) .AND. (STGDOY(11) .EQ. 0))THEN
...
!      TRSH = Part%TOPDM*1000. * 0.0   !HARVFRAC(2)
!      TRSH = Part%TOPDM*1000. + Out%TRASDM*1000.

      TRSH = CaneCrop % dw_lf ! Use a Harvest Index to control trash harvesting
...
!     Store Simulated and Measured data for this season.
        ...
        WRITE(Simulated(7),'(F8.2)')    CaneCrop%stk_h;     WRITE(Measured(7),'(A8)')X(7)
        ...
        WRITE(Simulated(13),'(F8.1)')   CaneCrop%fw_it_AG;  WRITE(Measured(10),'(A8)')X(11)
      ENDIF  
...
!      LABEL(1) = 'AELH'; VALUE(1) = AELH
!      LABEL(2) = 'SUCH'; VALUE(2) = SUCH
