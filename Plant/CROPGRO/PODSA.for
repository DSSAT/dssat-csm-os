...
!-----------------------------------------------------------------------
     ...
     &  WARNING, TIMDIF, CURV, TABEX
      ...
      CHARACTER*1   ISWWAT, ISWFWT, IDETL
      CHARACTER*2   CROP
      CHARACTER*3   TYPPDT, TYP1TMP, TYP2TMP, TYP3TMP 
      CHARACTER*6   ERRKEY
      ...
      INTEGER YRDOY, YRNR1, YRNR2, DAS, YRPLT, YRSIM
      INTEGER NDSET
      INTEGER TRIGGR

      INTEGER YEAR, DOY, DAP                           ! VSH
...
!      REAL PHTIM(365), PNTIM(365)
	    ...
	    REAL AVSWBAR (NCOHORTS), AVSWFAC (NCOHORTS),  TEMPAIR 
      REAL TSHR1, TSHR (NL), TSOIL (TS), PODINF,WTSHELL,PCINF

	    REAL ATEMP(NCOHORTS), STEMP(NCOHORTS), STEMPE(NCOHORTS),  
     & AFINFE(NCOHORTS), AFMASS(NCOHORTS),ASPMASS(NCOHORTS), 
     & TSOILA, WTSEED, AFCON, AFSUM, gPCTINF, gAFCONC, gAFSUM


	    REAL FN1TMP(4), FN2TMP(4),FN3TMP(4),R1, R2, R3, R4
!     P module
...
!     and daily aflatoxin per cohort 04/09/2024
      ...
      INPA = trim(CONTROL % PATHPE) // FILEP
      
      CALL GETLUN('INPA',  NINPA)
      OPEN (NINPA,FILE = INPA, STATUS = 'OLD',IOSTAT=ERR)
      
      SECTION = '*AFLAT'
      CALL FIND(NINPA, SECTION, LNUM, FOUND)
      
      
      IF (FOUND .EQ. 0) THEN
        CALL ERROR(SECTION, 42, INPA, LNUM)
      ELSE

       !Open (Unit=NINPA, File =INPA, Status ='Old')
       Read (NINPA, '(A200)') Header1
       Read (NINPA, 15) (FN1TMP(I), I=1, 4), R1, TYP1TMP 
       Read (NINPA, 15) (FN2TMP(I), I=1, 4), R2, TYP2TMP 
       Read (NINPA, 15) (FN3TMP(I), I=1, 4), R3, TYP3TMP 
       close (NINPA)
      ENDIF
      
  15   Format (5f6.0,3x, A3)
...
!-----------------------------------------------------------------------
        INQUIRE (FILE = OUTAC, EXIST = FEXIST)
        IF (FEXIST) THEN
          OPEN (UNIT = NOUTAC, FILE = OUTAC, STATUS = 'OLD',
     &      IOSTAT = ERRNUM)
          FIRST = .FALSE.
        ELSE
          OPEN (UNIT = NOUTAC, FILE = OUTAC, STATUS = 'NEW',
     &      IOSTAT = ERRNUM)
          FIRST = .TRUE.
        ENDIF
 ...
 152      Format ('   YRDOY    HOUR  SOIL TEMP(First Layer)')
        ...
        Write (NOUTAC, 1521)
1521    Format ('@YEAR DOY   DAS   DAP    PODINF     PODNO    SEEDNO'  ! VSH
     &        '   WTSHELL    WTSEED     PCINF     AFCON     AFSUM')
...
!AYSE
     ...
     &      CURV(TYPPDT,FNPDT(1),FNPDT(2),FNPDT(3),FNPDT(4),TGRO(I))
         TEMPAIR = TEMPAIR + TGRO(I)
        ENDDO
        TEMPOD = TEMPOD/REAL(TS)
! AF
        TEMPAIR  = TEMPAIR/REAL(TS)
! AF
...
! AF
                AVSWBAR(NPP) = (AVSWBAR(NPP) * (NAGE-1) + SWBAR)/NAGE
                AVSWFAC(NPP) = (AVSWFAC(NPP) * (NAGE-1) + SWFAC)/NAGE
! AF
              ENDIF
!-----------------------------------------------------------------------
...
! AF
!-----------------------------------------------------------------------
            PAGE = PHTIM(NR2TIM + 1) - PHTIM(NPP)      ! 2
	          NAGE = NR2TIM + 1 - NPP                  ! 3
C-----------------------------------------------------------------------
...
!           ASPMASS (NPP) =0.0
	       ...
	       ELSE                               
	         ...
	         AFINFE (NPP)= AFINFE(NPP)+ (SHELN(NPP)-AFINFE(NPP)) * R1
     &    * (1.0-SWBAR) * (1.0-SWFAC)
!     &    * (1.0-AVSWBAR(NPP)) * (1.0-AVSWFAC(NPP))
     &    *  CURV(TYP1TMP,FN1TMP(1),FN1TMP(2),FN1TMP(3),FN1TMP(4),ST(1))
!     &    *  (CURV('QDR', 22, 33, 35, 45, ST(1))) ! BASE, OPT1, OPT2, MAX TEMPERATURE
!     &    *  (CURV('QDR', 22., 33., 35., 45., 24.0)) ! BASE, OPT1, OPT2, MAX TEMPERATURE

! note:  Cole et al. 1984, showing peak to 35C.  Looks like curve above

 	     ASPMASS (NPP) = ASPMASS(NPP) + AFINFE(NPP)! 8
! 	     ASPMASS (NPP) = ASPMASS(NPP) + AFINFE(NPP) * ASPMASS(NPP)  ! 8
     &     * R2 
     &    * (1.0-SWBAR) * (1.0-SWFAC)
!     &    * R2 * (1.0-AVSWBAR(NPP)) * (1.0-AVSWFAC(NPP))
     &*  CURV(TYP2TMP,FN2TMP(1),FN2TMP(2),FN2TMP(3),FN2TMP(4),ST(1))
!     &    *  (CURV('QDR', 13, 28, 29, 46, ST(1)))        
!     &    *  (CURV('QDR', 13., 28., 29., 46., 24.0))        

! Graeme Wright, 11, 33, 34, 45C for growth of aspergillus colonies?
                 
	         AFMASS (NPP) = AFMASS (NPP) + AFINFE(NPP) * R3           ! 8
!	         AFMASS (NPP) = AFMASS (NPP) + ASPMASS(NPP) * R3           ! 8
     &     * (1.0-SWFAC)
!     &    * (1.0-SWBAR) * (1.0-SWFAC)
!     &    * (1.0-AVSWBAR(NPP)) * (1.0-AVSWFAC(NPP))
     &*  CURV(TYP3TMP,FN3TMP(1),FN3TMP(2),FN3TMP(3),FN3TMP(4),ST(1))
!     &    *  (CURV('QDR', 26, 28, 29, 32, ST(1)))         
!     &    *  (CURV('QDR', 26., 28., 29., 32., 24.0))         
...
! AF
	            WTSEED = WTSEED + WTSD(NPP) 
			        WTSHELL = WTSHELL + WTSHE(NPP)     !10
         
              PODINF =  PODINF + AFINFE (NPP)
              
              !WRITE(*,*) YRDOY, " BEFORE AFSUM", AFSUM
              
              AFSUM = AFSUM + AFINFE(NPP)* AFMASS(NPP)
              
              !WRITE(*,*) YRDOY, "  AFTER AFSUM", AFSUM
! AF      
              IF (PAGE .GE. LAGSD) THEN
                IF (SDDES(NPP) .GT. 0.0) THEN
...
! AF
	   IF (PODNO <0.0001) THEN
            PCINF =0.0
         ELSE
! kjb - put 100. in to make it real percentage
             PCINF = 100. * PODINF/PODNO
      ENDIF


	   IF (WTSEED <0.0001) THEN
            AFCON =0.0
         ELSE
! units WTSEED is g/m2.  If AFSUM is g, then AFCON is fraction  If ug, then AFCON is ppm
! if AFSUM is ng, then output AFCON is ppb
! want output as ppb, assumed multiply by 1000 x 1000 x 1000 if AFSUM is ng
             AFCON = AFSUM/WTSEED
             !WRITE(*,*) YRDOY, " AFSUM ", AFSUM, " WTSEED ", WTSEED, " AFCON ", AFCON
      ENDIF
       
       WRITE (NOUTAC,'(1X,I4,1X,I3.3,2(1X,I5), 8F10.2)') YEAR,DOY,DAS,   ! VSH
     &   DAP, PODINF,PODNO,SEEDNO, WTSHELL,WTSEED,PCINF, AFCON, AFSUM
      
       LABEL(1)  = 'PCTI'; VALUE(1)  = PCINF
       LABEL(2)  = 'AFCO'; VALUE(2)  = AFCON
