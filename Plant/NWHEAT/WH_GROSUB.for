...
! Need to output the ptnup and ctnup
     ...
     &      ASMDOT, DLAYR, DS, DTT, EO, EOP, ES, FILEIO, !Inp
     &      FracRts, ISTAGE, zstage,                  !Input
     &      KG2PPM, LL, NLAYR, nh4ppm, no3ppm,                !Input
     &      nwheats_kvalue,   !Input
     &      OZON7, pgdd, PLTPOP, PPLTD, rlv_nw, rtdep_nw, !Input
...
!     Diseased leaf area (cm2[leaf]/m2[ground]/d) - (Fabio 09/19/2018)
      REAL DISLA    
...
!     real pesw_nw(NL)! available water in layer
      ...
      real FO3      ! ozone concentration factor for photosynthesis
      REAL fr_intc_radn 
...
!     REAL plantwtmn(mxpart)!Replaced with "plwtmn"      nwheat
      ...
      REAL sen_la_p ! Prev Senesced leaf area, mm2/plant WP 07/21/2017
...
!        TF (31/08/2019) - Photosynthesis rate should be calculated over the non-diseased leaf area (XHLAI)
         ...
         AREAH  = MAX(0.,AREAH)
         XHLAI=AREAH/10000  
!         XHLAI: Healthy leaf area index used to compute
...
!          FO3 = (-0.001 * OZON7) + 1.025   !Ozone Intermediate cultivars
      ...
      if (swdef(photo_nw) .lt. 0.0001) then  ! to prevent dividing by 0
          PRFO3 = 1.0
      else
          PRFO3 = min(1.0, (FO3*rue_factor)/swdef(photo_nw)) ! ozone 
      endif
