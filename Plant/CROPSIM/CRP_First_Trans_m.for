        ...
        INTEGER,PARAMETER::KEYSTX = 9 ! Maximum number of key stages !TF*
    ...
    !    INTEGER :: FNUMREA                 ! File number,reads.out file     #          ! (From RunInit)   
        ...
        REAL    :: GGPHASE(4)              ! Grain set start,etc.,stages    #          ! (From SeasInit)
        ...
        REAL    :: LGPHASE(2)              ! Leaf growth phase start,end    #          ! (From SeasInit)
        ...
        REAL    :: SGPHASE(2)              ! Stem growth phase start,end    #          ! (From SeasInit)
        ...
        REAL    :: SSTH(SSX)               ! Secondary stage thresholds     du         ! (From SeasInit)
        ...
        INTEGER :: VERSIONCSCRP            ! Version #                      #          ! (From RunInit)   
        ...
        CHARACTER (LEN=1)   CFLNOUTPUTS   ! Control flag,detailed Noutputs            ! TF* (From RunInit)
        CHARACTER (LEN=1)   CFLPDATE      ! Control flag,planting date I,P            ! TF* (From RunInit)
        ...
        CHARACTER (LEN=8)   DATEWRITE     ! YEARDOY character string -> op            ! (From Output)  
        ...
        CHARACTER (LEN=250) FILEIO        ! Name of input file,after check            ! (From RunInit)  
        ...
        CHARACTER (LEN=6)   GSTAGEC       ! Growth stage                              ! (From Output)
        ...
        CHARACTER (LEN=5)   SSABV(SSX)    ! Secondary stage abbreviation              ! (From SeasInit)
        CHARACTER (LEN=5)   SSABVO(SSX)   ! Secondary stage abv,output                ! (From SeasInit)
        ...
        REAL          ADAPFR        ! Anthesis DAP+fraction          # ! (from Integrate)
        INTEGER       ADAPM         ! Anthesis,DAP,measured          d ! (From Output)
        INTEGER       ADATERR       ! Anthesis date error            d ! (From Output)
        INTEGER       ADATM         ! Anthesis date,measured         # ! (From Output)
        REAL          ADATT         ! Anthesis date from t file      YrDoy ! (From Output)
        REAL          ADAYEFR       ! Anthesis end,fraction of day   #
        REAL          AEDAPFR       ! Anthesis end date (DAP+fr)     # ! (from Integrate)
        INTEGER       AEDAPM        ! Anthesis end date,measured     # ! (From Output)
        ...
        REAL          CHWADOUT      ! Chaff weight for output        kg/ha ! (From Output)
        ...
        REAL          CNCTMP        ! Canopy N concentration,temp    % ! (From Output)
        REAL          CNPCMM        ! Canopy N,maturity,measured     % ! (From Output)
        ...
        REAL          DEADWAMM      ! Dead weight retained,measured  kg/ha ! (From Output)
        REAL          DGLF(LNUMX)   ! Days during which leaf growing #
        !REAL          DEPMAX        ! Maximum depth of soil profile  cm ! (from integrate)
        INTEGER       DRDAP         ! Double ridges date             dap ! (from integrate)
        REAL          DRF1          ! Double ridges factor 1         # ! (from integrate)
        REAL          DRF2          ! Double ridges factor 2         # ! (from integrate)
        REAL          DRF3          ! Double ridges factor 3         # ! (from integrate)
        REAL          DRSTAGE       ! Double ridges stage            # ! (from integrate)
        ...
        REAL          GFDAPFR       ! Grain filling start DAP+fr     # ! (from integrate)
        INTEGER       GFDAPM        ! Grain filling date,measured    # ! (From Output)
        REAL          GGPHASEDU(4)  ! Grain set start,etc.,stages    PVTt
        REAL          GLIGPC        ! Grain lignin content           %          ! (From SeasInit)
        REAL          GMPCH         ! Grain moisture percent,harvest #          ! (From SeasInit) 
        REAL          GNAM          ! Grain N at maturity            kg/ha ! (from integrate)
        REAL          GNAMM         ! Harvest N,mature,measured      kg/ha ! (From Output)
        REAL          GNOAMM        ! Grain #,mature,measured        #/m2  ! (From Output)
        REAL          GNOGMM        ! Grain#/group,mature,measured   #/gr ! (From Output)
        REAL          GNPCMM        ! Harvest N,mature,measured      %    ! (From Output)
        REAL          GNOGM         ! Grains/tiller (group),maturity #/gr  (from integrate)
        REAL          GNOPD         ! Grains per plant               #/p
        REAL          GNOPM         ! Grains per plant,maturity      #/p  (from integrate)
        ...
        REAL          GNOWTM        ! Grains/non-grain wt,maturity   #/g ! (from integrate)
        ...
        REAL          GWAHM         ! Grain weight,harvest,measured  kg/ha ! (From Output)
        REAL          GWAMM         ! Grain weight,maturity,measured kg/ha ! (From Output)
        ...
        REAL          GWUMM         ! Grain wt/unit,mat,measured     g ! (From Output)
        ...
        INTEGER       IESTG         ! Inflorescence emergence stage  # (from SeasInit)
        INTEGER       KEYSSNUM      ! Secondary key stage total #    # (from SeasInit)
        REAL          LAFR          ! Leaf area increase factor,rep  #
        REAL          LAFST         ! Leaf area factor change stage  #
        REAL          LAFSTDU       ! Leaf area factor change st,DU  # (from SeasInit)
        ...
        REAL          LFGSDU        ! Leaf growth start,d.units      # ! (from integrate)
        REAL          LFWTA         ! Leaf weight,anthesis           g/p ! (from integrate)
        REAL          LFWTAE        ! Leaf weight,anthesis end       g/p ! (from integrate)
        ...
        INTEGER       LLSTG         ! Last leaf fully expanded stage # (from SeasInit)
        ...
        REAL          LRETS         ! Stage --> dead leaves retained # (from SeasInit)
        REAL          LRETSDU       ! Stage --> dead leaves retained PVoCd
        REAL          LSHAR         ! Leaf sheath area/wt,end leaf   cm2/g (from SeasInit)
        REAL          LSHAV         ! Leaf sheath area/wt,veg.phases cm2/g (from SeasInit)
        ...
        REAL          LSHAW         ! Leaf sheath area/wt            cm2/g ! (from integrate)
        ...
        REAL          PDMTOHAR      ! Phase duration,mature->harvest deg.d (from SeasInit)
        ...
        REAL          RSPCLX        ! Reserves conc,leaves,max.      # (from SeasInit)
        ...
        REAL          RSWTA         ! Reserves weight,anthesis       g/p ! (from integrate)
        REAL          RSWTAE        ! Reserves weight,anthesis end   g/p ! (from integrate)
        ...
        !REAL          RTWTSGE       ! Root weight,stem growth end    g/p ! (From Output)
        REAL          RUESTG        ! Stage at which RUE changes     # (from SeasInit)
        ...
        REAL          SDRSPC        ! Seed reserves % of seed        # (from SeasInit)
        ...
        REAL          SGEDAPFR      ! Stem growth end DAP+fr         # ! (from integrate)
        INTEGER       SGEDAPM       ! Stem growth end date,measured  # ! (From Output)
        REAL          SGPHASEDU(2)  ! Stem growth phase start,end    Du
        REAL          SOILN         ! Soil inorganic N               kg/ha ! (From Output)
        REAL          SOILNF        ! Soil inorganic N,final         kg/ha ! (from integrate)
        ...
        REAL          STGEFR        ! Stem growth end time,fr day    # ! (from integrate)
        ...
        REAL          SSDAPFR(SSX)  ! Stage DAP+fr                   # ! (From integrate)
        INTEGER       SSDAPM(SSX)   ! Stage DAP,measured             # ! (From Output)
        REAL          SSDAYFR(SSX)  ! Stage fraction of day          # ! (From integrate)
        ...
        REAL          SSTG(SSX)     ! Secondary stage occurence,rstg # ! (From SeasInit)
        ...
        REAL          STWTA         ! Stem weight,anthesis           g/p ! (From integrate)
        REAL          STWTAE        ! Stem weight,anthesis end       g/p ! (From integrate)
        ...
        INTEGER       TILDAPM       ! Tillering date,measured        # ! (From Output)
        ...
        REAL          TMEANCAV      ! Temperature,mean,cycle av      C     ! (From integrate)
        REAL          TNUMAERR      ! Shoot #,error                  % ! (From Output)
        INTEGER       TNUMCOL       ! Treatment number column        # ! (From Output)
        REAL          TNUMPM        ! Tiller (+main stem) #,maturity #/p ! (From integrate)
        REAL          TNUMPMM       ! Shoot #,mature,measured        #/pl ! (From Output)
        REAL          TNUMT         ! Shoot number from t file       #/m2 ! (From Output)
        ...
        REAL          CHRSWAD       ! Chaff reserves                 kg/ha (from Integrate)
        ...
        !REAL          DAYLT         ! Daylength,6deg below horizon   h (from Integrate)
        ...
        INTEGER       SSDATM(SSX)   ! Stage date,measured            # ! (from Output)
        INTEGER       SSNUM         ! Secondary stage number         # (from SeasInit)
        REAL          SSPHE         ! Stem senesce phase end         # (from SeasInit)
        !REAL          STEMNGL       ! Stem N growth from leaves      g/p
        ...
        REAL          TDPHE         ! Tiller death end stage         # (from SeasInit)
        REAL          TDPHS         ! Tiller death start stage       # (from SeasInit)
        !REAL          TFLF(LNUMX)   ! Temp factor for leaf,average   #
        !REAL          TILBIRTHL(25) ! Tiller birth leaf number       #
        INTEGER       TILDAT        ! Tillering start date (YEARDOY) #
        REAL          TILPE         ! Tiller number growth end stg   # (from SeasInit)
        REAL          TILNOX        ! Shoot number,maximum           #/p (from SeasInit)
        !REAL          TKILL         ! Temperature for plant death    C
        !REAL          TLA(25)       ! Tiller leaf area produced      cm2
        !REAL          TLAS(25)      ! Tiller leaf area senesced      cm2
        !INTEGER       TLIMIT        ! Temp.limited grain gr (Days)   #
        REAL          TKFH          ! Start kill temp.fully hardened C (from SeasInit)
        REAL          TKUH          ! Start kill temp,unhardened     C (from SeasInit)
        ...
        REAL          VBASE         ! Vrn requirement before devment d (from SeasInit)
        REAL          VEEND         ! Vernalization effect end Rstge #  (from SeasInit)
        REAL          VEENDDU       ! Vernalization effect end DU    #  (from SeasInit)
        REAL          VEFF          ! Vernalization effect,max.reduc fr (from SeasInit)
        !REAL          VF            ! Vernalization factor 0-1       #
        REAL          VPEND         ! Vernalization process end Rstg # (from SeasInit)
        REAL          VPENDDU       ! Vernalization process end DU   # (from SeasInit)
        ...
        CHARACTER (LEN=13)  SSNAME(SSX)   ! Secondary stage names (from SeasInit)
        CHARACTER (LEN=1)   SSTYP(SSX)    ! Secoudary stage type (from SeasInit)
        CHARACTER (LEN=1)   EXAMINE(20)   ! Flag for examination of parts (from SeasInit)
