...
!***************************************************************************************************************************
    ...
    SUBROUTINE YCA_Out_PlGrow ( & 
        ...
        IF ((IDETG /= 'N'.AND.IDETL /= '0').OR.IDETL == 'A') THEN
            ...
            L = INT(BRSTAGE)
            WRITE (NOUTPG, FMT501)YEAR,DOY,DAS,DAP,TMEAN,BRSTAGEC,NINT(LNUM),PARIOUT,PARIUE,AMIN1(999.9,CARBOBEG*plantPopulation()), &
                LAIC,SAID,CAIC,NINT(TWAD),NINT(SDWADOUT),NINT(RWAD),NINT(CWAD),NINT(LLWADOUT),NINT(STWADOUT),NINT(HWAD), &
                HIAD, NINT(RSWAD),SENTOPLITTERAC,SENROOTC,RSCD*100.0,NINT(BRNUMST(L)*PLTPOP), & !issue 50 ! DA 25MAY2018 apex number multiplied by plant population
