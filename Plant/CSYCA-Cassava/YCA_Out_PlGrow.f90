    ...
    SUBROUTINE YCA_Out_PlGrow ( & 
        ...
        IF ((IDETG /= 'N'.AND.IDETL /= '0').OR.IDETL == 'A') THEN
            ...
            WRITE (NOUTPG, FMT501)YEAR,DOY,DAS,DAP,TMEAN,BRSTAGEC,LNUM,PARIOUT,PARIUE,AMIN1(999.9,CARBOBEG*plantPopulation()), &
                LAIC,SAID,CAIC,NINT(TWAD),NINT(SDWADOUT),NINT(RWAD),NINT(CWAD),NINT(LLWADOUT),NINT(STWADOUT),NINT(HWAD), NINT(HWFD), &
                DMHD,HIAD, NINT(RSWAD),SENTOPLITTERAC,SENROOTC,RSCD*100.0,BRNUMSHM, & !issue 50
