...
void READ_WSTAT(char *FILEWW, int *ERRCODE){
  ...
  if(file.is_open()){
    ...
    if(hdsection && datasection){
      //Error: Header has variables with no value.
      if(hddata.size() > dtdata.size()) {*ERRCODE = 29; return;}
      
      for(int i = 0; i < hddata.size(); i++){
        //std::cout << "VEC FIO WSTAT: " << hddata[i] << ": " << dtdata[i] << std::endl;
        flexibleio->setString("WTH", hddata[i], dtdata[i]);
      }
    }
    else{
...
void READ_WTH_Y2_4K(char *FILEWW, char *RTYPE, int *YRDOY, 
  ...
  std::ifstream file;
  int hdcount, datecol, nrow, fwd, lwd, nrec, century, yeardoy, yydoy,yy,doy;
