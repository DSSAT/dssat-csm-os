...
!-----------------------------------------------------------------------
     ...
     &  WINDSP,XLAT,XLONG,CCO2,RHUM, VAPR, DCO2, OZONX
...
!               Relative humidity at TMIN (or max rel. hum) (%)
              ...
              CASE('OZONX')   !Maximum daily surface ozone (ppb)
                READ(LINE(C1:C2),*,IOSTAT=ERR) OZONX
                IF (ERR .NE. 0) OZONX = -99.0
            END SELECT
          ENDDO
...
!       Print confirmation that header was found to INFO.OUT
     ...
     &        'PAR','RHUM','VAPR','VPRS','DCO2','CO2','OZONX')
            IM = IM + 1
