...
!              RWU(L) = 0.0   ROOTWU, not here.
            ...
            DO L = 1, NLAYR
              ...
              RLV(L) = AMIN1(0.20*PLANTS/DLAYR(L),0.5) ! Teff
...
!          STRESSN = AMAX1 (SI4(3),SI4(4))    !not used
          ...
          PANIWT  = 0.0
          G1FAC   = AMIN1 (1.0,G1/50.0)!Teff
          G1FAC   = AMIN1 (1.0,G1/7500.0)!Tef
          PANFAC  = 0.65*G1FAC  ! Teff
          PANFAC  = 0.45*G1FAC  !Tef
          IF (STRHEAT .LT. 1.0) THEN
             PANFAC = PANFAC * STRHEAT
          ENDIF
          !STRESSW = AMAX1 (SI2(3),SI2(4))
          PANIWT  = (MSTMWT*0.4+MLFWT*0.20)*G1FAC*(1.0+0.5*STRESSW) !+.5bias water stress -0.5 for Teff
...
!          PANNO  = (TILNO+1.0)*PLANTS    !Not used
    ...
    ! TL      = EXP(-0.85*ROWSPC*PLSC)               ! .65
      ...
      RUEA    = 3.2   ! Tef
      RUEX    = 0.65
      PCARB   = RUEA*PAR**RUEX/EPLANTS*(1.0-AMIN1(Y1,Y2)) ! Teff
      ...
      PCARB = PCARB*PCO2
      PRFT  = 1.0-0.0025*((0.25*TMIN+0.75*TMAX)-26.0)**2   !need to lower so temp is 10 for prft=0 mar17 ! Teff
...
!        PC = 0.56 + 0.068*CUMPH  ! J.Padilla
     ...
     &		        AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) ! Teff
     ...
     &			       AMIN1 (TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) !Tef
             ...
             IF (TCARBO .LT. 0.0) THEN    ! NEW FOR Teff AND TEF
     ...
     &              AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) +
     &              (0.65*TCARB1)/0.0055
         ! GRORT   = CARBO*0.15 !Teff
          GRORT   = CARBO*(0.20-SWFAC*0.05)  !TEF
          MGROLF  = MPLAG*0.0060 !Teff
          MGROLF  = MPLAG*0.0060 !Tef
  !       MGROSTM = 0.0015*DTT*AMIN1(TURFAC,TEMF)+0.35*TCARB1!Teff
  !       MGROSTM = AMIN1 (MGROSTM,0.65*CARBO) !Teff
 ...
 !   &        		AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) ! Teff
     ...
     &        		AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) !Tef
          ...
          IF (GRF .GE. TILCAR .AND. GRF .LE. CARBO-GRORT) THEN 
             !TILCAR  = (MGROSTM*0.5+MGROLF*0.3)*G3    !FOR Teff!
           ! TILCAR  = 0.0    !FOR TEF
             MGROLF  = (MGROLF*0.5)/G3   ! REDUCED FOR TEF WAS 0.7 FOR Teff
     ...
     &		         AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES) !Tef
         ...
         !GRF    = TILCAR     !US
          IF (GRF .GT. CARBO-GRORT) THEN
             ...
             TCARBO  = GRF
          ENDIF
       !  IF (TCARBO .NE. GRF) THEN                !Teff
       !     ADDTIL  = GRF     - TCARBO            !Teff
       !     SHORAT  = MGROLF  / (MGROLF+MGROSTM)  !Teff
       !     TPART   = SHORAT  * ADDTIL            !Teff
       !     MGROLF  = MGROLF  - TPART             !Teff
       !     MGROSTM = MGROSTM - (ADDTIL-TPART)    !Teff
       !     TCARBO  = GRF                         !Teff
       !     MPLAG   = MGROLF/0.0060*              !Teff
    !&                 AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES)!Teff
    !        MPLAG   = MGROLF/0.0060*
   ! &                 AMIN1(TURFAC,TEMF,AGEFAC,PSTRES2,KSTRES)!Tef
   !      ENDIF
...
!          SLAN   = PLA*(0.015+0.95*(SUMDTT/(P4+P5))**2)      
        ...
        CASE (5)  ! End of main culm grain filling
          ...
          TCARB1  = CARBO
          CARBO  = CARBO*(1.0-0.45*(SUMDTT/P5)) ! FOR Teff ONLY
!          SLAN   = PLA*(0.015+0.85*((SUMDTT+P4)/(P4+P5))**2)
          !0.85--> 0.25   US 
          SLAN   = (PLA-SENLA)*(0.005+0.01*((SUMDTT+P4)/(P4+P5))**2)   
          TMPFIL = 1.0

          IF (TEMPM .LT. 15.0) THEN     !20 FOR Teff 15 FOR TEF
             TMPFIL = (TEMPM-7.0)/8.0  !/13 FOR Teff 8 FOR TEF
          ...
          IF (SUMDTT .LT. P5*0.90) THEN
             ...
             IF (GNO .GT. GPP) THEN
                GROGRN = 0.0
             ENDIF
             GRORT   = CARBO*(0.2-0.1*SWFAC)  !Teff
