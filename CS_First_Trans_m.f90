Module CS_First_Trans_m
    ...
    !REAL    :: AFLF(0:LNUMX)         ! CH2O factor for leaf,average   #          ! (From SeasInit) !LPM 23MAR15 to have a value by canopy level 
    REAL    :: AFLF(0:PSX,0:LCNUMX)         ! CH2O factor for leaf,average   #          ! (From SeasInit)
    ...
    REAL    :: DALF(0:LNUMX)         ! Days during which leaf active  d          ! (From SeasInit)  
    ...
    REAL    :: DGLF(0:LNUMX)         ! Days during which leaf growing #          ! (From SeasInit)  
    ...
    REAL    :: DSLF(0:LNUMX)         ! Days during which leaf senesng #          ! (From SeasInit)  
    ...
    INTEGER :: KEYPS(KEYSTX)         ! Principal key stage number     #          ! (From SeasInit)  
    ...
    REAL    :: LAGEG(0:LNUMX)        ! Leaf age increment             C.d        ! (From Growth)    
    REAL    :: LAGEP(0:LNUMX)        ! Leaf age (phyllochrons),lf pos #          ! (From SeasInit)  
    REAL    :: LAGETT(0:LNUMX)       ! Leaf age at leaf position      C.d        ! (From SeasInit)  
    REAL    :: LAGL(LNUMX)           ! Leaf area growth,shoot,lf pos  cm2/l      ! (From SeasInit)  
    ...
    REAL    :: LAISTG(20)            ! Leaf area index,specific stage #          ! (From SeasInit)  
    ...
    REAL    :: LAP(0:LNUMX)          ! Leaf area at leaf position     cm2/p      ! (From SeasInit)  
    ...
    REAL    :: LAPOTX(0:PSX,LCNUMX)  ! Leaf area potentials,maxima    cm2/l      ! (From SeasInit)  
    ...
    REAL    :: LATL(0:PSX,LNUMX)     ! Leaf area,shoot,lf#,potential  cm2/l      ! (From SeasInit)  
    REAL    :: LATL2(0:PSX,LCNUMX)   ! Leaf area,shoot,lf#,+h2o,n,tem cm2/l      ! (From SeasInit)  
    REAL    :: LATL3(0:PSX,LCNUMX)   ! Leaf area,shoot,lf#,+assim.    cm2/l      ! (From SeasInit)  
    REAL    :: LATL4(0:PSX,LCNUMX)   ! Leaf area,shoot,lf#,+assim.+N  cm2/l      ! (From SeasInit)  
    REAL    :: LATLPOT(0:PSX,LCNUMX) ! Leaf area,shoot,leaf,pot       cm2/l      ! (From Growth)    
    REAL    :: LATLPREV(0:PSX,LCNUMX)! Leaf area,shoot,leaf,prev.     cm2/l      ! (From Growth)    
    ...
    INTEGER :: LBIRTHDAP(LCNUMX)     ! DAP on which leaf initiated    #          ! (From Growth)    
    INTEGER :: LCNUM                 ! Leaf cohort number (inc.grow)  #          ! (From SeasInit)  
    REAL    :: LCOA(LCNUMX)          ! Leaf cohort area               cm2        ! (From SeasInit)  
    REAL    :: LCOAS(LCNUMX)         ! Leaf cohort area senesced      cm2        ! (From SeasInit)  
    INTEGER :: LDEATHDAP(LCNUMX)     ! DAP on which leaf 100% dead    #          ! (From Integrate) 
    ...
    REAL    :: NFLF(0:PSX,LCNUMX)    ! N factor for leaf,average      #          ! (From SeasInit) !LPM 23MAR15 Adjusted to consider two dimensions  
    REAL    :: NFLF2(0:PSX,LCNUMX)   ! N factor for leaf area adj     #          ! (From SeasInit) !LPM 23MAR15 Adjusted to consider two dimensions 
    REAL    :: NFLFP(LNUMX)          ! N factor phs leaf,average      #          ! (From SeasInit)  
    ...
    INTEGER :: PSDAP  (PSX)          ! Stage DAP                      #          ! (From SeasInit)  
    REAL    :: PSDAPFR(PSX)          ! Stage DAP+fr                   #          ! (From Integrate) 
    INTEGER :: PSDAPM (PSX)          ! Stage DAP,measured             #          ! (From SeasInit)  
    INTEGER :: PSDAT  (PSX)          ! Stage YrDoydate                #          ! (From SeasInit)  
    INTEGER :: PSDATM (PSX)          ! Stage date,measured            #          ! (From Output)    
    REAL    :: PSDAYFR(PSX)          ! Stage fraction of day          #          ! (From SeasInit)  
    ...
    REAL    :: PTF                   ! Partition fraction to tops     #          ! (From SeasInit)  
    ...
    REAL    :: TFDLF(LNUMX)          ! Temp factor,dev for leaf,av    #          ! (From SeasInit)  
    ...
    REAL    :: TFGLF(LNUMX)          ! Temp factor,gr for leaf,av     #          ! (From SeasInit)  
    ...
    REAL    :: WFLF(0:PSX,LCNUMX)    ! H2O factor for leaf,average    #          ! (From SeasInit) !LPM 23MAR15 Change to consider two dimensions
    ...
    CHARACTER (LEN=5)   PSABV(PSX)    ! Principal stage abbreviation              ! (From SeasInit) 
    CHARACTER (LEN=5)   PSABVO(PSX)   ! Principal stage abv,output                ! (From SeasInit) 
    CHARACTER (LEN=13)  PSNAME(PSX)   ! Principal stage names                     ! (From SeasInit) 
    CHARACTER (LEN=1)   PSTYP(PSX)    ! Principal stage type                      ! (From SeasInit) 
