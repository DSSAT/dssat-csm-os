...
!=======================================================================
      ...
      SUBROUTINE CHECK_Y4K_WTH(CONTROL,MEWTH,LUNEXP,DSSATP,PATHEX,FILEX, &              
      ...
      CHARACTER*1   BLANK, UPCASE, MEWTH, RNMODE
      CHARACTER*3   PROCOD
      CHARACTER*6   ERRKEY,SECTION
      CHARACTER*5   FINDH
      CHARACTER*8   WSTA,WSTA1,FILEW4
      CHARACTER*12  NAMEF,LastFileW,FILEX,FILEW,FILEWG,FILEWC,FILEWTH
      CHARACTER*78  MSG(4)
      CHARACTER*80  PATHEX,CHARTEST,PATHWT,PATHWTC,PATHWTG,PATHWTW
      ...
      INTEGER       ICASAF,UNIT, I  !,LINWTH,LUNWTH
      INTEGER       SDATE,PDATE,YRDOY
      INTEGER       LNFLD,TLNFLD
      INTEGER       LNSIM,TLNSIM
      INTEGER       LNPLT,TLNPLT
      INTEGER       YEARDOY,YEAR,YR,DOY,YRSIM
      
      DATA LastFileW /" "/

      PARAMETER (ERRKEY = 'Y4KWTH')
      PARAMETER (BLANK = ' ')
        ...
        IF(IFIND .NE. 0) THEN
          ...
          IF(IFIND .EQ. 1) THEN
            ...
            DO
              ...
              IF(ISECT .EQ. 1) THEN
                READ (CHARTEST,'(I3,9X,2A4)', IOSTAT=ERRNUM) TLNFLD,WSTA,WSTA1
      ...
      IF(LNSIM .GT. 0 .AND. SimLevel) THEN
        ...
        IF(IFIND .NE. 0) THEN
          DO
            ...
            !FO - Find the specific header to search for MEWTH
            FINDH ='N MET'
            CALL HFIND(LUNEXP,FINDH,LINEXP,IFIND)
            IF(IFIND .EQ. 1) THEN
              ...
              CALL IGNORE2(LUNEXP,LINEXP,ISECT,CHARTEST)  
              IF(ISECT .EQ. 1) THEN
                READ (CHARTEST,'(I2,17X,A1)', IOSTAT=ERRNUM) TLNSIM,MEWTH
                IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,11,FILEX,LNUM)
                IF (MEWTH .EQ. BLANK) CALL ERROR (ERRKEY,10,FILEX,LINEXP)            
              ELSE
...
!-----------------------------------------------------------------------    
      CALL WTHFDIR(CONTROL,MEWTH,FILEX,WSTA,WSTA1,YRDOY,      & ! Input
                   DSSATP,PATHEX,                             & ! Input
...
!-----------------------------------------------------------------------
      ...
      IF (PATHL .LE. 1) THEN
         FILEWW = FILEWTH
       ELSE
         FILEWW = PATHWT(1:(PATHL-1)) // FILEWTH
      ENDIF
      
      !FO - Process FILEW if last FILEW is different.
      IF(LastFileW /= FILEWTH) THEN
        ...
        IF(FEXIST) THEN
          ...
          IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,18,FILEWTH,0)
        ELSE
          CALL ERROR (ERRKEY,18,FILEWTH,0)
        ENDIF
        ...
        DO WHILE (ERRNUM .EQ. 0)
          ...
          ELSEIF(ICASAF .EQ. 1) THEN
            ...
            IF(LINE(1:1) .EQ. '@' .AND. INDEX(LINE, 'DATE') .GT. 0) THEN
                ...
                IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,17,FILEWTH,0)
                
                EXIT
            ELSE IF(LINE(1:1) .EQ. '@' .AND. INDEX(LINE, 'YEAR') .GT. 0) THEN
                !FO - Read ICASA format
                READ (UNIT,'(I5,1X,I3)', IOSTAT=ERRNUM) YEAR, DOY 
                IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,17,FILEWTH,0)
