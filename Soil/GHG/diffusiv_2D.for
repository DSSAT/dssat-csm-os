...
!=======================================================================
      subroutine DayCent_diffusivity(dD0, swv, soilprop)      
!      the variable dDO_fc is manually calculated for the moment

      use Cells_2D
      IMPLICIT NONE

      real, dimension(MaxRows,MaxCols), intent(out):: dD0
...
!         pfc = water content as percent of DUL
          pfc = SWV(L,J) / DUL(L) * 100.   
          if (pfc >= 100.) then
            VFRAC = (SWV(L,J) - DUL(L)) / POROSer
...
!         dD0_DayCent(L) = MAX(0.0, (TP8/1.E7 + TP7 * TP6))
          dD0(L,J) = MAX(0.0, (TP8/1.E7 + TP7 * TP6))
