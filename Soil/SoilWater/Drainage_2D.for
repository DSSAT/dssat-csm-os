C=======================================================================
C  Drainage_2D, Subroutine, C.H.Porter
C  Calculates 2D drainage and diffusion for each soil cell 
C-----------------------------------------------------------------------
C  REVISION       HISTORY
C  10/28/2008 CHP Written based on WATBAL
!  08/21/2009 CHP Change to sub-hourly, variable time step. Remove MGAR.
!  08/15/2011  Rename LatFlow to LatFlow_ts
!              Drainage will be end at min(LIMIT_2D,  NLayr)
!              Limit vertical Flow not making next layer above saturation
!              LatFlow_ts due to Deficit is calculate in waterTable_2D, thus removed from this subroutine
!              Calculate LatFlow_ts due to drainage from layer LIMIT_2D
!              Below LIMIT_2D, set subroutine output water content equal to subroutine input water content 
!              If water table depth is too small, the array visualizer shows super high SW around LIMIT_2D,
!                here only fixed to avoid the SW in layer LIMIT_2D to go to above SAT 
C-----------------------------------------------------------------------
C  Called by: Watbal2D module
C=======================================================================
